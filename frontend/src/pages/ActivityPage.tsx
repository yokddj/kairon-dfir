            ...
            {(operationsQuery.data?.operations ?? []).map((operation: ActivityOperation) => {
              ...
              <article key={`${operation.kind}-${operation.id}`} className="rounded-2xl border border-line bg-abyss/60 p-4">
                ...
                <div className="mt-3 flex flex-wrap gap-2 text-xs">
                  <button type="button" className="rounded-xl border border-line px-3 py-1.5 text-muted disabled:opacity-50" disabled={!canContinue} onClick={() => uploadSessionId && navigate(`/cases/${operation.case_id}/evidence?tab=evidences&resume_session=${uploadSessionId}`)}>Continue wizard</button>
                  <button type="button" className="rounded-xl border border-line px-3 py-1.5 text-muted disabled:opacity-50" disabled={!evidenceId} onClick={() => evidenceId && navigate(`/cases/${operation.case_id}/evidence?tab=processing&evidence_id=${evidenceId}`)}>Open evidence</button>
                  <button type="button" className="rounded-xl border border-line px-3 py-1.5 text-muted disabled:opacity-50" disabled={!evidenceId} onClick={() => evidenceId && navigate(`/cases/${operation.case_id}/evidence?tab=processing&evidence_id=${evidenceId}`)}>Open processing</button>
