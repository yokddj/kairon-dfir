...
| Shellbags | yes | yes | yes | SBECmd CSV output | stable partial | yes | yes if timestamped | User Activity | yes if selected | SBECmd CSV is parsed and indexed. Raw NTUSER.DAT/UsrClass.dat hive parsing is not implemented; detected raw hives are discovery-only. | Raw hive Shellbags parser. |
