...
| Incident Timeline | stable | Curated reportable incident chronology | default, high-signal sources only | `/cases/{case_id}/incident-timeline` | incident timeline endpoints | Not an all-events timeline; draft can be validation-seeded in demo/training cases. | Improve curation workflow. |
