...
from app.ingest.normalizer import base_document, normalize_file, normalize_row, parse_timestamp
...
def test_process_tree_source_fields_omit_raw_and_search_text_by_default() -> None:
    from app.services.process_tree import _process_tree_source_fields

    fields = _process_tree_source_fields(include_raw_samples=False, include_raw_xml=False)
    assert "raw" not in fields
    assert "search_text" not in fields
    assert "raw_summary" in fields


def test_process_tree_source_fields_include_raw_when_requested() -> None:
    from app.services.process_tree import _process_tree_source_fields

    fields = _process_tree_source_fields(include_raw_samples=True, include_raw_xml=True)
...
def test_build_process_tree_bundle_keeps_summary_coherent_with_filtered_graph(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("app.services.process_tree._search_scope_events", lambda context, size, extra_filters=None: (events, len(events), {}))
...
def test_build_process_tree_bundle_filters_connected_component(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("app.services.process_tree._search_scope_events", _fake_search)
...
def test_velociraptor_discovery_prioritizes_native_amcache_and_shimcache_over_generic_registry_candidates(
    ...
    # The SYSTEM hive is independently parsed for three unrelated purposes
    # (service, shimcache, and Host Facts -- see
    # app.ingest.velociraptor.discovery._detect_priority_registry_raw_candidate),
    # so it legitimately produces three native candidates over the same
    # path, not one -- collect all of them rather than relying on
    # candidates_by_path's last-one-wins collapsing for this path.
    system_candidates = {candidate.category: candidate for candidate in discovery.candidates if candidate.original_path == "uploads/triage/C%3A/Windows/System32/config/SYSTEM"}
    assert system_candidates.keys() == {"service", "shimcache", "windows_system_hive_facts"}
    assert system_candidates["shimcache"].artifact_type == "shimcache"
    assert system_candidates["shimcache"].supported is True
    assert system_candidates["windows_system_hive_facts"].artifact_type == "windows_system_hive_facts"
...
def test_process_tree_scope_search_request_caps_page_size_to_search_request_limit(tmp_path: Path) -> None:
    ...
    search_request = _build_scope_search_request(context, page_size=min(max(context.max_events_per_type * 30, 300), 200))
...
def test_parallel_evidence_ref_prefers_user_provided_host_without_overwriting_detected_host() -> None:
    ...
    assert result["host_id"] == "4e4e4e4e-1111-4111-8111-4e4e4e4e4e4e"


def test_parallel_normalized_artifact_extracts_windows_host_facts(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("app.workers.tasks._update_artifact_row_isolated", lambda *args, **kwargs: True)  # noqa: ARG005
    monkeypatch.setattr("app.workers.tasks.bulk_index_events_with_report", lambda *args, **kwargs: {"attempted": True, "success": True, "documents_indexed": 1, "warnings": []})  # noqa: ARG005

    def fake_normalize_file(*args, **kwargs):  # noqa: ARG001
        document = base_document("case-1", "evidence-1", "artifact-1", {}, {"parser": "evtxecmd_csv"})
        document["windows"]["computer_field"] = "WS01.megacorp.local"
        return [document]

    monkeypatch.setattr("app.workers.tasks.normalize_file", fake_normalize_file)

    host_facts_calls: list[dict] = []

    def fake_safe_create_host_facts_isolated(**kwargs):
        host_facts_calls.append(kwargs)
        return None

    monkeypatch.setattr("app.workers.tasks._safe_create_host_facts_isolated", fake_safe_create_host_facts_isolated)

    tracker: dict[str, dict] = {}
    _process_parallel_normalized_artifact(
        evidence_ref={"id": "evidence-1", "case_id": "case-1", "host_id": "5f5f5f5f-1111-4111-8111-5f5f5f5f5f5f", "detected_host": None, "detected_user": None},
        artifact_info={
            "name": "Security.evtx",
            "artifact_type": "windows_event",
            "source_path": "Windows/System32/winevt/Logs/Security.evtx",
            "parser": "evtxecmd_csv",
            "path": "/tmp/Security.evtx",
        },
    ...
    assert len(host_facts_calls) == 1
    call = host_facts_calls[0]
    assert call["host_id"] == "5f5f5f5f-1111-4111-8111-5f5f5f5f5f5f"
    fact_types = {doc["host_fact"]["fact_type"] for doc in call["documents"]}
