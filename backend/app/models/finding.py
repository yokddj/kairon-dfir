...
class Finding(UUIDMixin, TimestampMixin, Base):
    ...
    linked_event_id: Mapped[str | None] = mapped_column(String(255), nullable=True, index=True)
    source_view: Mapped[str | None] = mapped_column(String(128), nullable=True, index=True)
    source_route: Mapped[str | None] = mapped_column(String(1024), nullable=True)
    ...
    source_snapshot_json: Mapped[dict | None] = mapped_column(JSONVariant, nullable=True)
