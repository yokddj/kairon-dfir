...
class QcowImageAdapter:
    ...
    def _detect_variant(self, path: Path) -> str | None:
        ...
        if header and header[:4] == b"QFI\xfb":
            return "qcow2"
        if header and header[:4] == b"QFI\xfe":
            return "qcow"
    ...
    def detect(self, path: Path, companions: list[Path]) -> dict[str, Any] | None:
        ...
        if header and header[:4] in {b"QFI\xfb", b"QFI\xfe"}:
    ...
    def inspect(self, path: Path, companions: list[Path]) -> dict[str, Any]:
        ...
        snapshot_count = 0
        if info:
            snapshots = info.get("snapshots") or info.get("format-specific", {}).get("data", {}).get("snapshots") or []
