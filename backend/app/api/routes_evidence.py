...
def _actor_label(current_user: User | None, fallback: str | None = None) -> str:
    return fallback or _current_user_id(current_user) or "analyst"
...
REGISTRY_HIVE_NAMES = {"software", "system", "sam", "security", "default", "ntuser.dat", "usrclass.dat", "amcache.hve", "components"}


def _registry_candidate_path(entry: dict[str, Any]) -> str:
...
def _registry_candidate_basename(entry: dict[str, Any]) -> str:
    raw = _registry_candidate_path(entry)
    if not raw:
        return ""
    return raw.replace("/", "\\").rstrip("\\").rsplit("\\", 1)[-1].lower()
...
def _registry_hive_name(entry: dict[str, Any]) -> str:
    basename = _registry_candidate_basename(entry)
    if basename in REGISTRY_HIVE_NAMES:
        return basename.upper() if "." not in basename else basename
    return str(entry.get("artifact_type") or "registry_hive")
...
def build_registry_diagnostic(item: Evidence, db: Session) -> dict[str, Any]:
    ...
    for container in (
        ...
        for entry in container if isinstance(container, list) else []:
            if isinstance(entry, dict) and _is_registry_hive_candidate(entry):
                candidates.append(dict(entry))
    hive_candidates = list({str(entry.get("source_path") or entry.get("relative_path") or entry.get("path") or entry.get("original_path") or entry.get("display_name") or entry): entry for entry in candidates}.values())
    ...
    return {
        "evidence_id": item.id,
        "case_id": item.case_id,
        "available": bool(hive_candidates and backend_available),
        "status": status,
        "hives_present": bool(hive_candidates),
        "hive_count": len(hive_candidates),
...
def upload_evidence(
    ...
    if memory_upload:
        ...
        try:
            evidence = register_memory_evidence(upload_state.id, db=db)
            if assigned_host_id:
...
class EvidenceCaseHostPatchRequest(BaseModel):
    host_id: str | None = None
...
@router.patch("/api/cases/{case_id}/evidence/{evidence_id}/host", response_model=EvidenceRead)
def patch_case_evidence_host(
    case_id: str,
    evidence_id: str,
    payload: EvidenceCaseHostPatchRequest,
    db: Session = Depends(get_db),
    current_user: User | None = Depends(get_optional_user),
) -> Evidence:
    evidence = _get_case_evidence(db, case_id, evidence_id)
    actor = _actor_label(current_user, payload.analyst)
    actor_user_id = _current_user_id(current_user)
    if payload.host_name and payload.host_id:
        raise HTTPException(status_code=400, detail="Provide host_id or host_name, not both")
    host_created = False
    ...
    return _set_evidence_host_assignment(
        db,
        evidence,
        ...
        method="analyst_assigned" if target_host_id else "analyst_unassigned",
        confidence="high",
        host_created=host_created,
    )
...
def unassign_evidence_host(evidence_id: str, db: Session = Depends(get_db), current_user: User | None = Depends(get_optional_user)):
    evidence = db.get(Evidence, evidence_id)
    if not evidence:
        raise HTTPException(status_code=404, detail="Evidence not found")
    evidence = _set_evidence_host_assignment(
        ...
        actor_user_id=_current_user_id(current_user),
        actor=_actor_label(current_user),
        reason="Analyst unassigned",
        method="analyst_unassigned",
    )
...
def get_evidence_search_summary(evidence_id: str, db: Session = Depends(get_db)) -> dict:
    ...
    summary["registry_diagnostic"] = build_registry_diagnostic(item, db)
    return summary
...
@router.get("/api/evidences/{evidence_id}/registry-diagnostic")
def get_evidence_registry_diagnostic(evidence_id: str, db: Session = Depends(get_db)) -> dict:
    item = db.get(Evidence, evidence_id)
    if not item:
        raise HTTPException(status_code=404, detail="Evidence not found")
    return build_registry_diagnostic(item, db)
