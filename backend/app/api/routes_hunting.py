...
def hunting_patch_finding(case_id: str, finding_id: str, payload: dict = Body(...), db: Session = Depends(get_db)) -> dict:
    ...
    allowed = {"title", "description", "confidence", "severity", "tags", "related_hosts", "evidence_id", "linked_evidence_id", "linked_host_id", "linked_artifact_id", "linked_artifact_family", "linked_artifact_type", "linked_event_id", "source_view", "source_route", "source_timestamp", "source_label", "source_summary", "source_snapshot_json", "created_by"}
    ...
    for key, value in payload.items():
        if key in allowed:
            ...
            if key == "related_hosts" and isinstance(value, list):
                value = [str(host).strip() for host in value if str(host).strip()]
